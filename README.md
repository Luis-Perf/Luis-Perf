<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=220&color=0:0f172a,50:312e81,100:2563eb&text=Luis%20Felipe&fontSize=48&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Backend%20Developer%20•%20Laravel%20•%20PHP%20•%20Sistemas%20Financeiros&descAlignY=58" />

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Inter&weight=500&size=24&duration=3000&pause=1200&color=8B5CF6&center=true&vCenter=true&width=900&lines=Desenvolvedor+Back-end+com+foco+em+PHP+e+Laravel;Construindo+APIs+robustas+e+sistemas+escaláveis;Experiência+com+plataformas+financeiras+e+integrações;Full+Stack+com+base+em+QA%2C+dados+e+IA)](https://git.io/typing-svg)

</div>

---

## 👨‍💻 Sobre mim

Sou **Desenvolvedor Back-end Pleno**, com foco em **PHP, Laravel, APIs REST e sistemas financeiros**.

Atuo no desenvolvimento e manutenção de soluções em produção, com experiência em **middleware de pagamentos**, integrações com múltiplas adquirentes, webhooks, antifraude, bancos de dados e arquitetura de sistemas.

Também possuo experiência Full Stack, automação de testes, Python, IA e desenvolvimento de interfaces modernas com React e Angular.

---

## ⚙️ Stack principal

<div align="center">

<img src="https://skillicons.dev/icons?i=php,laravel,python,nodejs,react,angular,js,ts,mysql,postgres,mongodb,redis,docker,aws,git,github&theme=dark" />

</div>

---

## 🧠 Principais competências

| Back-end | Front-end | Bancos de dados | DevOps & Ferramentas |
|---|---|---|---|
| PHP 8 • Laravel 12 • Python • Node.js • APIs REST • JWT • Webhooks | React • Angular • TypeScript • JavaScript • UI responsiva | MySQL • PostgreSQL • MongoDB • Redis | Git • GitHub • Laravel Forge • CI/CD • Selenium |

---

## 🏦 Experiência atual

Atualmente trabalho com desenvolvimento back-end em uma plataforma financeira white-label, atuando em:


✔ Middleware financeiro multi-adquirente
✔ Roteamento dinâmico por método de pagamento
✔ Integração com adquirentes e gateways
✔ Fluxos de split de pagamento
✔ Cálculo de taxas, antecipação, saque e reserva financeira
✔ Webhooks bidirecionais
✔ Integração antifraude
✔ Correção de bugs críticos em produção
✔ Documentação técnica

---

## 🚀 Projetos em destaque

<div align="center">

<a href="https://github.com/Luis-Perf/NLP_practice">
  <img height="150em" src="https://github-readme-stats.vercel.app/api/pin/?username=Luis-Perf&repo=NLP_practice&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/Luis-Perf/gestor_obras">
  <img height="150em" src="https://github-readme-stats.vercel.app/api/pin/?username=Luis-Perf&repo=gestor_obras&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/Luis-Perf/consumo_API">
  <img height="150em" src="https://github-readme-stats.vercel.app/api/pin/?username=Luis-Perf&repo=consumo_API&theme=tokyonight&hide_border=true" />
</a>

<a href="https://github.com/Luis-Perf/landing-page-template">
  <img height="150em" src="https://github-readme-stats.vercel.app/api/pin/?username=Luis-Perf&repo=landing-page-template&theme=tokyonight&hide_border=true" />
</a>

</div>

---

## 📈 Estatísticas do GitHub

<div align="center">

<img height="170em" src="https://github-readme-stats.vercel.app/api?username=Luis-Perf&show_icons=true&theme=tokyonight&hide_border=true&count_private=true&include_all_commits=true" />

<img height="170em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Luis-Perf&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" />

</div>

---

## 🔥 Sequência de contribuições

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com?user=Luis-Perf&theme=tokyonight&hide_border=true" />

</div>

---

## 📊 Gráfico de atividade

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Luis-Perf&bg_color=0d1117&color=8b5cf6&line=8b5cf6&point=ffffff&area=true&hide_border=true" />

</div>

---

## 🎯 Interesses profissionais

- Desenvolvimento Back-end
- Sistemas financeiros
- APIs e integrações
- Arquitetura de software
- Plataformas SaaS
- Microsserviços
- Cloud
- Inteligência Artificial aplicada a negócios

---

## 🌍 Idiomas

- Português — Nativo
- Inglês — Intermediário/avançado, com leitura técnica fluente
- Espanhol — Intermediário
- Japonês — Básico

---

## 📬 Contato

<div align="center">

<a href="https://www.linkedin.com/in/luis-felipe-perfeito/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="mailto:luisperfeito.1@hotmail.com">
<img src="https://img.shields.io/badge/E--mail-181717?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

</div>

---

<div align="center">

### 🚀 Construindo software real, uma entrega por vez.

</div>
